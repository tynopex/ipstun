...
fn sub_bytes_inv(x: [u8,..16]) -> [u8,..16]
{
    let mut o: [u8,..16] = unsafe { mem::uninitialized() };
...
fn shift_rows_inv(x: [u8,..16]) -> [u8,..16]
{
    [
        x[ 0],x[13],x[10],x[ 7],
        x[ 4],x[ 1],x[14],x[11],
...
fn mix_columns(x: [u8,..16]) -> [u8,..16]
{
    ...
    for i in iter::range_step(0,16,4)
    {
        ...
        o[i+3] = gfm(x[i+0], 3) ^ gfm(x[i+1], 1) ^ gfm(x[i+2], 1) ^ gfm(x[i+3], 2);
    }

    o
}

fn mix_columns_inv(x: [u8,..16]) -> [u8,..16]
{
    let mut o: [u8,..16] = unsafe { mem::uninitialized() };
    ...
    for i in iter::range_step(0,16,4)
    {
        ...
        o[i+3] = gfm(x[i+0],11) ^ gfm(x[i+1],13) ^ gfm(x[i+2], 9) ^ gfm(x[i+3],14);
    }
...
fn dec_block(d: [u8,..16], kv: KeySchedule) -> [u8,..16]
{
    ...
    for i in range(1,1+NROUND).rev()
    {
        ...
        if i < 10
        {
            st = mix_columns_inv(st);
        }

        st = shift_rows_inv(st);
        st = sub_bytes_inv(st);
    }
...
mod test
{
    ...
    fn test_key_sched()
    {
        ...
        for i in range(1,1+NROUND)
        {
            ...
            assert_eq!(key_schedule(ki, i), ko);
        }
    }
